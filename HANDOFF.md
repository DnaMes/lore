...
nightly run worktree was removed. Local Lore `main` is clean at `b0e84a2`, equal
to `github/main`; GitHub has 51 open issues, zero open PRs, and only remote `main`.
No code tests were run for the failed nightly.
...
On AI-Workstation, Lore's local `main` ref tracks `origin/main` at `b0e84a2`,
while the canonical submodule checkout remains detached at `9ffb3ee`; the
`ai-stack` superproject records `d3953c1`. The superproject is 17 commits behind
